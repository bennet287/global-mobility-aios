...
type DecisionsSceneRead = {
  latest: LivingOrganizationSceneLatest | null;
  loading: boolean;
  error: string | null;
  loadedAt: string | null;
};

export function useV2DecisionsScene() {
  const [read, setRead] = useState<DecisionsSceneRead>({ latest: null, loading: true, error: null, loadedAt: null });
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setRead((current) => ({
        ...
        loading: false,
        error: denied ? "Access to Decision records was denied." : caught instanceof Error ? caught.message : "Decision source unavailable.",
