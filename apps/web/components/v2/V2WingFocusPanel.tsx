...
export type V2WingFocusPanelProps = {
  ...
  readonly onSelectCharacter?: (positionKey: string) => void;
};
            ...
            {zone.characters.map((character) => {
              ...
              return (
                <li key={character.positionKey}>
                  {onSelectCharacter ? (
                    <button
                      aria-label={`Inspect ${character.title || character.positionKey}`}
                      aria-pressed={selected}
                      className={styles.characterButton}
                      data-character-selected={String(selected)}
                      onClick={() => onSelectCharacter(character.positionKey)}
                      type="button"
                    >
                      {content}
                    </button>
                  ) : (
                    <div className={styles.characterStatic}>{content}</div>
                  )}
                </li>
              );
            })}
