...
import {
  CoverageTrancheAssistantConfig,
  CoverageTrancheAssistantResult,
  createJurisdictionCoverageBatch,
  getCoverageTrancheAssistantConfig,
...
export default function GlobalIntelligencePage() {
  ...
  const [trancheAssistantConfig, setTrancheAssistantConfig] = useState<CoverageTrancheAssistantConfig | null>(null);
  const [trancheAssistantBatchId, setTrancheAssistantBatchId] = useState("");
  const [trancheAssistantCodes, setTrancheAssistantCodes] = useState("");
  const [trancheAssistantQueueBaselines, setTrancheAssistantQueueBaselines] = useState(false);
  const [trancheAssistantBusy, setTrancheAssistantBusy] = useState(false);
  const [trancheAssistantResult, setTrancheAssistantResult] = useState<CoverageTrancheAssistantResult | null>(null);
  const [assistantDraftCopied, setAssistantDraftCopied] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [healthResult, intelligence, registryResult, authorityResult, sourceResult, worklistResult, batchResult, assistantConfig] = await Promise.all([
      ...
      setCoverageWorklist(worklistResult); setCoverageBatches(batchResult.batches); setTrancheAssistantConfig(assistantConfig);
      ...
      if (!trancheAssistantBatchId && batchResult.batches.length) setTrancheAssistantBatchId(batchResult.batches[0].id);
    } catch (err) { setError(err instanceof Error ? err.message : "Could not load global intelligence"); }
    finally { setLoading(false); }
  }, [authorityFilter, confidenceFilter, coverageFilter, coverageWorkGap, coverageWorkRegion, freshnessFilter, initialAssertionBatchId, materialityFilter, reviewStateFilter, trancheAssistantBatchId, windowDays]);
  ...
  async function reviewSourceCertification(decision: "approved" | "rejected") {
    ...
    try {
      ...
      await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Could not review source certification"); }
    finally { setAssessmentBusy(false); }
  }
  ...
  async function runTrancheAssistant(dryRun: boolean) {
    if (!trancheAssistantBatchId) return;
    const codes = trancheAssistantCodes
      .split(/[\s,;]+/)
      .map((value) => value.trim().toUpperCase())
      .filter(Boolean);
    if (!codes.length) {
      setError("Enter at least one two-letter jurisdiction code.");
      return;
    }
    setTrancheAssistantBusy(true); setError(null);
    try {
      const result = await prepareCoverageTranche(trancheAssistantBatchId, {
        alpha2_codes: codes,
        dry_run: dryRun,
        queue_eligible_baselines: trancheAssistantQueueBaselines,
        include_candidate_assertions: true,
        max_candidate_lines: 8,
      });
      setTrancheAssistantResult(result);
      if (!dryRun && result.queued_baselines > 0) await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Could not prepare the selected coverage tranche"); }
    finally { setTrancheAssistantBusy(false); }
  }

  function useAssistantDraft(item: CoverageTrancheAssistantResult["items"][number]) {
    const draft = item.candidate_assertion;
    if (!draft) return;
    setInitialAssertionBatchId(trancheAssistantResult?.batch_id || trancheAssistantBatchId);
    setInitialAssertionCode(draft.alpha2_code);
    setInitialAssertionDomain(draft.domain);
    setInitialAssertionTitle(draft.title);
    setInitialAssertionRuleKey(draft.rule_key);
    setInitialAssertionStatement(draft.statement);
    setInitialAssertionExcerpt(draft.evidence_excerpt);
    setInitialAssertionRationale(draft.rationale);
    setInitialAssertionConfidence(String(draft.confidence));
    setAssistantDraftCopied(draft.alpha2_code);
    window.requestAnimationFrame(() => {
      initialAssertionSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      window.requestAnimationFrame(() => initialAssertionTitleRef.current?.focus({ preventScroll: true }));
    });
  }
          ...
          <section className="panel coverage-operations-panel">
            ...
            <div className="coverage-tranche-assistant">
              ...
              <div className="coverage-tranche-assistant-form">
                <label>Evidence batch<select value={trancheAssistantBatchId} onChange={(event) => { setTrancheAssistantBatchId(event.target.value); setTrancheAssistantResult(null); }}><option value="">Select batch</option>{coverageBatches.slice(0, 8).map((batch) => <option key={batch.id} value={batch.id}>{batch.name}</option>)}</select></label>
                <label>Jurisdiction codes<input placeholder="DE or DE, CA" value={trancheAssistantCodes} onChange={(event) => setTrancheAssistantCodes(event.target.value.toUpperCase())} /></label>
                <label className="coverage-tranche-checkbox"><input type="checkbox" checked={trancheAssistantQueueBaselines} onChange={(event) => setTrancheAssistantQueueBaselines(event.target.checked)} /> Queue only explicitly selected approved baselines in apply mode</label>
                <div className="coverage-tranche-actions"><button className="button secondary" disabled={!trancheAssistantConfig?.enabled || trancheAssistantBusy || !trancheAssistantBatchId || !trancheAssistantCodes.trim()} onClick={() => void runTrancheAssistant(true)}>{trancheAssistantBusy ? "Preparing…" : "Prepare dry run"}</button><button className="button primary" disabled={!trancheAssistantConfig?.enabled || trancheAssistantBusy || !trancheAssistantQueueBaselines || !trancheAssistantBatchId || !trancheAssistantCodes.trim()} onClick={() => void runTrancheAssistant(false)}>Apply selected baseline queues</button></div>
                <small>Dry run is the default. The assistant never approves assessments, certifies sources, creates assertions, publishes rules, changes snapshots, or makes a coverage claim.</small>
              </div>
              {trancheAssistantResult && <div className="coverage-tranche-results">
                <div className="coverage-tranche-summary"><span>{trancheAssistantResult.selected_count} selected</span><span>{trancheAssistantResult.would_queue_baselines.length} baseline queues eligible</span><span>{trancheAssistantResult.queued_baselines} queued</span><span>{trancheAssistantResult.dry_run ? "dry run" : "applied"}</span></div>
                {trancheAssistantResult.items.map((item) => <article key={item.batch_item_id}>
                  <div><div><span>{item.alpha2_code}</span><h4>{item.jurisdiction_name || item.alpha2_code}</h4></div><StatusBadge value={item.stage} /></div>
                  <p>{item.next_action}</p>
                  {item.review_packet.official_source && <small>{item.review_packet.official_source.name} · {item.review_packet.official_source.url}</small>}
                  {item.snapshot_analysis && <div className="coverage-tranche-quality"><span>Snapshot quality {item.snapshot_analysis.quality_score}/100</span><span>{titleCase(item.snapshot_analysis.classification)}</span><span>{item.snapshot_analysis.keyword_hits.length} keyword groups</span></div>}
                  {item.candidate_assertion ? <div className="coverage-tranche-draft"><strong>Constrained draft suggestion</strong><p>{item.candidate_assertion.statement}</p><blockquote>{item.candidate_assertion.evidence_excerpt}</blockquote><small>Human editing and independent review remain mandatory. No assertion has been created.</small><button type="button" className="button secondary" onClick={() => useAssistantDraft(item)}>{assistantDraftCopied === item.alpha2_code ? "Draft copied — opening form" : "Copy draft into assertion form"}</button></div> : item.snapshot_analysis && <InlineNotice label="No safe draft generated" detail="The snapshot is missing substantive immigration text or is too navigation-heavy. Select a better official page and capture a new approved baseline instead of forcing a rule." tone="warn" />}
                </article>)}
              </div>}
            </div>
            <div className="coverage-initial-rules" ref={initialAssertionSectionRef}>
              ...
              <div className="coverage-initial-rule-grid">
                <div className="coverage-initial-rule-form">
                  ...
                  <label className="coverage-initial-rule-wide">Title<input ref={initialAssertionTitleRef} placeholder="Human-readable assertion title" value={initialAssertionTitle} onChange={(event) => setInitialAssertionTitle(event.target.value)} /></label>
          ...
          <section className="panel">
            ...
            {selectedRegistryEntry && <div className="registry-assessment-workspace">
              ...
              <div className="registry-certification-section">
                ...
                {selectedRegistryEntry.approved_supplemental_source_certifications?.map((certification) => <article key={certification.id} className="registry-approved-assessment"><div><StatusBadge value="approved" /><strong>Supplemental official source certified</strong></div><p>{certification.evidence_notes}</p><small>Scope: {certification.certification_scope} · reviewed by {certification.reviewed_by || "unknown"} · domains: {certification.coverage_domains.join(", ")}</small></article>)}
                {selectedRegistryEntry.pending_source_certification ? <article className="registry-pending-assessment"><div><StatusBadge value="pending_review" /><strong>{selectedRegistryEntry.pending_source_certification.certification_scope === "primary_immigration" ? "Primary source certification pending" : "Supplemental source certification pending"}</strong></div><p>{selectedRegistryEntry.pending_source_certification.evidence_notes}</p><small>Scope: {selectedRegistryEntry.pending_source_certification.certification_scope} · proposed by {selectedRegistryEntry.pending_source_certification.proposed_by}. A different authenticated reviewer must decide.</small><textarea placeholder="Certification review notes" value={certificationReviewNotes} onChange={(event) => setCertificationReviewNotes(event.target.value)} /><div><button className="button primary" disabled={assessmentBusy || certificationReviewNotes.trim().length < 3} onClick={() => void reviewSourceCertification("approved")}>Approve {selectedRegistryEntry.pending_source_certification.certification_scope === "primary_immigration" ? "certification" : "supplemental source"}</button><button className="button danger" disabled={assessmentBusy || certificationReviewNotes.trim().length < 3} onClick={() => void reviewSourceCertification("rejected")}>Reject</button></div></article> : selectedAuthorities.length && jurisdictionSources.length ? <div className="registry-assessment-form">
