...
test("wing detail placed presentations remain selectable and open Employee Inspector", () => {
  includesAll(
    focusPanel,
    [
      "onSelectCharacter?: (positionKey: string) => void",
      "selectedPositionKey?: string | null",
      "aria-pressed={selected}",
      "onSelectCharacter(character.positionKey)",
      "Inspect ${character.title || character.positionKey}",
    ],
    "wing focus panel character controls",
  );
  includesAll(
    wingWorkspace,
    [
    ...
    "wing detail employee continuity",
  );
});
