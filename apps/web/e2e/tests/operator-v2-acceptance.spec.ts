...
const operatorRoutes = [
  { href: "/", label: "Work", heading: "What requires professional attention now?" },
  { href: "/operator/v2/profiles", label: "Profiles", heading: "Profile truth stays versioned, consent-aware, and evidence-linked." },
  { href: "/operator/v2/pathways", label: "Pathways", heading: "Pathway truth stays governed, versioned, and evidence-linked." },
  { href: "/operator/v2/evidence", label: "Evidence", heading: "Evidence stays reviewable, source-linked, and human-controlled." },
  { href: "/operator/v2/communication", label: "Communication", heading: "Client communication stays reviewable, governed, and explicitly non-sending in the current MVP." },
  { href: "/operator/v2/tools", label: "Tools", heading: "Specialist tools stay governed, contextual, and explicitly human-controlled." },
] as const;
...
for (const width of [1280, 390]) {
  test(`Phase 10I Operator primary destinations remain navigable and bounded at ${width}px`, async ({ page }) => {
    ...
    for (const route of operatorRoutes) {
      ...
      if (width === 390) {
        const navOverflow = await nav.evaluate((element) => getComputedStyle(element).overflowX);
