...
def _mark_failed_if_current_owner(
    session: Session,
    *,
    tenant_key: str,
    work_item_id: UUID,
    ...
    if attempt is None or attempt.work_item_id != work.id:
        return False
    if attempt.status != "running" or work.status != "running":
        return False
    if attempt.execution_token != expected_execution_token or work.execution_token != expected_execution_token:
        return False
    ...
    if (
        ...
        or current.writer != actor
        or current.execution_token != expected_execution_token
    ):
        # A newer fence owns the attempt. The superseded worker may surface its error,
        # but it may not mutate canonical execution/work state after losing ownership.
        return False
