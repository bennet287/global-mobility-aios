...
def _validate_rate(value: float, *, field: str) -> float:
    ...
    if resolved < 0 or resolved > 1:
        raise InvalidTransition(f"{field} must be between 0 and 1")
    return resolved
...
def _scope_statement(
    *,
    ...
    return select(CapabilityAutonomyPromotionPolicy).where(
        ...
        CapabilityAutonomyPromotionPolicy.position_key == position_key,
        CapabilityAutonomyPromotionPolicy.capability_key == capability_key,
        CapabilityAutonomyPromotionPolicy.context_scope == context_scope,
        CapabilityAutonomyPromotionPolicy.from_autonomy_level == from_autonomy_level,
        CapabilityAutonomyPromotionPolicy.target_autonomy_level == target_autonomy_level,
        CapabilityAutonomyPromotionPolicy.evidence_policy_version == evidence_policy_version,
    )
...
def _validated_policy_revision(
    ...
    *,
    ...
    for field in (
        ...
        value = float(getattr(policy, field))
    ...
    if policy.min_recovery_success_rate is not None:
        if policy.min_recovery_success_rate < 0 or policy.min_recovery_success_rate > 1:
            raise AutonomyPromotionPolicyIntegrityError("promotion policy recovery rate is invalid")
    ...
    if position is None or position.position_key != policy.position_key:
    ...
    if (
        activity.activity_type != AUTONOMY_PROMOTION_POLICY_ACTIVITY_TYPE
...
def capability_autonomy_promotion_policy_snapshot(
    ...
    *,
    ...
    """Return the validated current Board promotion policy and append-only history."""
    ...
    for index, row in enumerate(rows, start=1):
        if (
            ...
            or row.position_key != position_key
            or row.capability_key != capability_key
            or row.context_scope != context_scope
            or row.from_autonomy_level != from_level.value
            or row.target_autonomy_level != target_level.value
            or row.evidence_policy_version != evidence_policy_version
        ):
            raise AutonomyPromotionPolicyIntegrityError("promotion policy scope drifted")
        revisions.append(
