...
def _approved_item_context(
    ...
    # Source-only supplemental batches intentionally omit a duplicate
    # batch-local assessment. Reuse the latest independently approved
    # assessment for the same jurisdiction only when no local assessment
    # was supplied. A local pending or rejected assessment is never bypassed.
    if not item.source_certification_id:
        raise ValueError(
            "Initial rule assertion requires a source certification review record"
        )
    if not item.official_source_id:
        raise ValueError(
            "Initial rule assertion requires an onboarded official source"
        )
    ...
    if not assessment or assessment.status != "approved":
        raise ValueError(
            "Immigration-rule relationship must be independently approved first"
        )
    if not certification or certification.status != "approved":
        raise ValueError(
            "Authority/source certification must be independently approved first"
        )
    if not jurisdiction or not source:
        raise ValueError(
            "Coverage item jurisdiction or official source could not be resolved"
        )
    if (
        assessment.jurisdiction_id != jurisdiction.id
        or certification.jurisdiction_id != jurisdiction.id
    ):
        raise ValueError(
            "Coverage review records do not belong to the selected jurisdiction"
        )
    ...
    if (
        not supplemental_scope
        and not source_only_supplement
        and assessment.official_source_id not in {None, source.id}
    ):
        raise ValueError(
            "Immigration assessment is linked to a different official source"
        )
    if certification.official_source_id != source.id:
        raise ValueError(
            "Source certification is linked to a different official source"
        )
