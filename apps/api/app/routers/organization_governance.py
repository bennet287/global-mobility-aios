...
from app.services.organization_governance import (
    ...
    delegate_product_work,
    delegate_security_operations_work,
    delegate_security_work,
...
def create_work_item(payload: WorkItemCreate, request: Request, session: Session = Depends(get_session)) -> OrganizationalWorkItem:
    ...
    elif payload.department.strip().lower() == "product" and runtime_available:
        delegate_product_work(session, work)
    elif payload.department.strip().lower() == "security" and runtime_available:
        delegate_security_work(session, work)
    elif payload.department.strip().lower() == "security operations" and runtime_available:
        delegate_security_operations_work(session, work)
    elif payload.department.strip().lower() == "marketing" and runtime_available:
        delegate_marketing_work(session, work)
    elif payload.department.strip().lower() == "finance" and runtime_available:
        delegate_finance_work(session, work)
    elif payload.department.strip().lower() == "communications" and runtime_available:
