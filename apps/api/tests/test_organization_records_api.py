...
def test_openapi_and_phase_architecture_boundaries() -> None:
    ...
    assert "0084_organization_agent_lifecycle.py" in migration_names
    assert "0085_runtime_budget_cost_metering.py" in migration_names
    assert not any(
        name[:4].isdigit() and int(name[:4]) > 85
