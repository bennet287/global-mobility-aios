...
The driver splits each ``_index.md`` at the first ``***``. Above it, the parts
``wiki update`` regenerates -- its frontmatter keys and the auto-generated link
list -- are taken from ours, and the remaining authored frontmatter keys get a
real three-way merge; the block *below* (hand-written content) is a real
three-way merge via ``git merge-file``. The driver recombines the halves and
exits nonzero when either merge left conflicts.
...
__all__ = [
    'test_clean_merge_takes_ours_above_and_merges_below',
    'test_above_taken_from_ours_with_divergent_link_blocks',
    'test_below_conflict_leaves_file_unmerged_with_markers',
    'test_thematic_break_below_is_not_a_separator',
    'test_empty_index_takes_ours',
...
def test_clean_merge_takes_ours_above_and_merges_below(
    tmp_path: pathlib.Path,
) -> None:
    """A clean merge keeps ours' above verbatim and three-way-merges below.

    Ours and theirs regenerate different link blocks (above); theirs also appends
    a line below. The result must be byte-identical to ours' above plus the
    three-way-merged below -- proving the recombination introduces no drift.
    """
...
def test_above_taken_from_ours_with_divergent_link_blocks(
    tmp_path: pathlib.Path,
) -> None:
    """Ours' above wins even when theirs adds links; below still merges.
    ...
    ours' unique link survives, theirs' link is dropped (above is regenerated by
    ``wiki update``), and theirs' below addition is picked up.
    """
    result = _three_way_merge(
        tmp_path / 'repo',
    ...
    assert '[[b|b]]: beta' not in result.text  # theirs' above dropped
...
def test_empty_index_takes_ours(tmp_path: pathlib.Path) -> None:
    """An index with no below content (``***`` as the last line) takes ours.

    Empty wiki/memory indexes end with ``***`` and nothing after it. The below
    merge of three empty bodies is clean, and ours' above is taken -- so theirs'
    regenerated links are dropped (``wiki update`` rebuilds them post-merge).
    """
    result = _three_way_merge(
        tmp_path / 'repo',
        base=_index((), ''),
...
def test_added_by_both_empty_base_merges_below(
    ...
    with an empty base. The driver must keep ours' above and three-way-merge the
    below against the empty base: identical below additions resolve clean, while
    divergent ones conflict. This is the one odd-looking input that is genuinely
    reachable -- added-by-both is a normal merge -- so it must behave correctly.
    """
    returncode, text = _run_driver(
        tmp_path,
        ours=_index(('[[a|a]]: alpha',), ours_below),
    ...
    assert '[[b|b]]: beta' not in text  # theirs' above dropped (regenerated later)
    if expect_clean:
        assert returncode == 0
        assert '<<<<<<<' not in text
    else:
...
def test_stray_separator_in_frontmatter_preserves_take_ours(
    tmp_path: pathlib.Path,
) -> None:
    """A bare ``***`` *inside* the frontmatter must not mis-split the file.

    The split skips the leading ``--- ... ---`` block before searching for
    ``^***$``, so a stray ``***`` in a multi-line ``desc`` is ignored and the real
    link block is taken from ours -- the take-ours-above guarantee holds: ours'
    link survives and theirs' does not leak through a mistaken "below" merge.
    """
    ...
    assert '[[a|a]]: alpha' in text  # ours' link kept -- take-ours-above honored
...
# ------ end-to-end take-ours-above safety (the design contract)


def test_take_ours_above_link_drop_is_restored_by_wiki_update(
    tmp_path: pathlib.Path,
) -> None:
    """Take-ours-above drops theirs' link at merge time, but never loses data.

    The driver takes ours' above-region verbatim, so a link only *theirs* added is
    discarded when an ``_index.md`` is merged. This is safe by design: the link
    block above ``***`` is a *derived view* that ``wiki update`` rebuilds from the
    page files on disk, and the node lifecycle runs ``wiki update`` after every
    merge. This drives the real cycle -- ours and theirs each add a distinct page,
    the driver merges the index (dropping theirs' link), yet theirs' page file
    merges in as an ordinary file, so the post-merge ``wiki update`` restores the
    link from ground truth. This is the contract that makes take-ours-above
    correct rather than data loss; it would fail if a dropped link were not
    reconstructable.
    """
    ...
    # merge theirs -- the driver resolves wiki/_index.md by take-ours-above
    ...
    # the post-merge wiki update rebuilds the link block from the page files,
    # restoring the dropped link -- so take-ours-above never loses data
    _wiki(wiki_dir, 'update')
    restored = index.read_text(encoding='utf-8')
    assert '[[a|a]]' in restored
    assert '[[b|b]]' in restored  # theirs' link is back, derived from b.md
    assert '[[c|c]]' in restored
