...
Phase 16.1 runtime provider-usage metering and Phase 16.2 deterministic failure classification/retry policy are sealed. Provider/model/token observations and `estimated_cost_usd` are diagnostic runtime evidence, not billing truth. Retry remains fail-closed: only classified provider transport failures retry. PR #171 seals cooperative AgentRun soft-timeout handling by reusing the existing Celery 240-second soft / 300-second hard limits; soft timeout is terminal and non-retryable. PR #173 seals stale-running reconciliation after the 300-second hard limit plus a 60-second grace window, using existing AgentRun + AuditLog truth and explicitly recording `cause_inferred=false` rather than claiming every stranded run was definitely hard-killed. PR #176 seals explicit AgentRun cancellation on the existing AgentRun/Celery/AuditLog boundary: queued work becomes `cancelled`, running work becomes `cancel_requested` until cooperatively observed, revoke remains non-terminating, and cancellation never claims rollback of already-performed effects. PR #179 seals a per-attempt provider ledger for controlled AgentRuns; PR #181 extends the same ledger to every direct model-call path without inventing AgentRuns for other contexts. PR #183 seals an opt-in provider call-count ceiling with an admin authorization boundary and atomic reservation before direct model calls. Unenrolled providers retain prior behavior; failed and unknown-outcome attempts consume a slot. PR #185 independently adds optional DeepSeek/Moonshot generated-token request caps; Gemini's current compatibility path has no verified cap. PR #187 exposes a read-only direct-model evidence report: partial estimates and unknown actual spend stay separate, and manually populated billed values without invoice provenance remain unverified. PR #189 preserves optional provider completion IDs for later correlation and inventories identified external API calls; neither proves an invoice or paid-tool charge. Itemized billing evidence, verified vendor cost contracts and a Board-granted monetary budget are still absent. PR #167 records the System-1/orchestration decision: Jev and Laya remain benchmark candidates only; Google AX remains deferred as a possible execution substrate until AIOS owns canonical timeout/cancellation, hard runtime budgets, actual cost metering, circuit breakers and reconciliation. Deterministic AIOS policy remains authoritative. No duplicate runtime state should be created merely to support lifecycle signals.

PR #192 closes only stranded AgentRun-linked provider attempts with terminal execution evidence; PR #193 adds a provider-scoped circuit for admin-enrolled call allocations, tripped by three consecutive classified transport failures and reopened only by an audited admin reset. PR #194 adds explicit request-local owner-completion evidence for the in-house consultant, and PR #196 extends the same accepted operation-key plus `AuditLog` owner-completion pattern to business advisory. A still-`started` attempt for either explicitly wired owner may reconcile to `outcome_unknown` only after durable owner-completion evidence keyed by the existing operation identity. Other request-local paths without that explicit signal remain untouched by age alone. None of these controls measures vendor charges, infers a provider outcome from owner completion, or releases a consumed call slot for an unknown outcome. Unenrolled providers and external tools remain outside the provider circuit.

Phase 17 is sealed through the controlled-agent prompt-boundary slice. PR #198 bounds authenticated-session replay lifetime; PR #200 makes debug routes admin-only; PR #203 makes credential-bearing connector configuration and health checks admin-only; PR #205 validates webhook HTTPS/public-address destinations and pins connection-time egress to a prevalidated public IP; PR #206 separates regulatory classifier policy/output contract from lower-trust source/diff evidence; PR #207 applies the same system-vs-untrusted-evidence boundary to business advisory; and PR #208 applies it to the existing controlled-agent role-card/provider path while removing the duplicate schema copy from the user envelope. These controls reuse existing auth, connector, provider, role-card, deterministic validation and human-review boundaries; they do not create a second security/authority/tool-control state or grant model content new authority.
...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; the provider-call attempt ledger is sealed for controlled AgentRuns by PR #179 and for every direct model-call path by PR #181; opt-in admin-governed cumulative provider call capacity is sealed by PR #183; optional DeepSeek/Moonshot per-call output-token caps are sealed by PR #185; read-only cost-evidence visibility is sealed by PR #187; provider completion identity plus the identified API external-call inventory are sealed by PR #189; AgentRun-linked stranded provider-attempt reconciliation is sealed by PR #192; enrolled-provider circuit admission is sealed by PR #193; explicit request-local owner-completion evidence is sealed for the in-house consultant by PR #194 and business advisory by PR #196. Other request-local paths still require their own real owner-completion signal before automatic reconciliation; age alone remains insufficient. Authoritative itemized provider billing evidence and a verified per-attempt join, plus real vendor cost contracts for paid tools, remain prerequisites for monetary allocation. Gemini output-cap compatibility remains separately unverified. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.

Phase 16.3F cost-exposure inventory from the current code (classification of external boundaries, **not** proof that a vendor bills each call):

| External boundary | Existing operation evidence | Billing attribution gap |
| --- | --- | --- |
| DeepSeek, Moonshot and Gemini chat completions (`llm_client.py`) | `ProviderCallAttempt` for each direct model-call attempt, with optional provider completion ID and observed tokens | No verified invoice line, account/tier linkage, or provider-specific billable cost; an ID alone is a correlation handle. Retries and unknown outcomes may incur charges. |
| Automation SMTP and webhook dispatch (`automation_connector.py`) | `AutomationDelivery` attempt count/status plus pre-use and result `AuditLog` | No monetary cost evidence. SMTP and webhook `provider_message_id` values are generated by the adapter, not verified vendor billing IDs; health-check egress has no delivery attempt. |
| Monitored public HTTP source retrieval (`source_retrieval.py`) | `SourceRetrievalRun` with retry identity, status and bytes | No evidence that the source charges per request and no metered charge; keep outside actual billed spend unless a paid source contract is supplied. |
| UN M49 registry fetch (`jurisdiction_registry.py`) | `JurisdictionRegistryRelease` for accepted imports | A fetch is not a release and has no per-request cost record; no verified paid source contract. |
| OpenBao secret resolution (`core/secrets.py`) | Configuration/secret access, outside the application attempt ledger | Infrastructure request cost is unknown; no evidence of per-access billing. |

This inventory covers the identified API external-call paths, not all infrastructure, compute, storage or human-review cost. Before source-linked reconciliation can set actual billed spend, obtain an authoritative itemized billing source, document its account/currency/line identity and join key for each provider, match it to exactly one attempt (including retries and unknown outcomes), reject duplicates and credits without explicit treatment, and preserve the source/provenance. Aggregate invoices without a per-call join remain unapportioned; never allocate them by token estimates or synthetic connector IDs. Paid-tool coverage must be assessed per real vendor contract before enabling a monetary budget.
...
Sealed Phase 17 slices now establish the initial boundaries without creating parallel security state. PR #198 bounds signed-session replay lifetime. PR #200 removes `/debug` from public access and reuses existing admin role policy. PR #203 restricts credential-bearing automation connector configuration/status/health checks to admins while ordinary delivery dispatch keeps its existing operator gate. PR #205 closes the identified webhook destination and connection-time escape path by requiring HTTPS, rejecting embedded credentials and any non-public DNS answer, pinning the request to a prevalidated public IP while retaining original Host/TLS identity, and disabling environment proxy/redirect escape paths. PR #206 keeps the regulatory classifier/output contract in the trusted system prompt while treating source metadata and unified-diff text as untrusted evidence. PR #207 applies the same trust separation to business advisory, preserving deterministic risk/prohibited-conduct gates. PR #208 applies it to the existing controlled-agent role-card/provider path, removes the duplicate schema copy from the lower-trust user envelope, and proves with an adversarial provider-payload regression that injected context cannot enter the trusted system message. CodeQL from PR #202 remains a scan rather than a substitute for these runtime controls.

The next Phase 17 selection is evidence-led: inventory remaining model-input/output crossings and the native-skill/tool provenance plus allowed/denied-tool entitlement path, then close the smallest concrete gap using the existing canonical owner. Prompt separation alone does not authorize tools, external mutation, canonical truth changes or bypass deterministic/human-review gates. Skill declarations likewise do not grant credentials, permission, authority or autonomy. Supply-chain integrity, provenance integrity, broader adversarial regression and incident-response work remain open until separately evidenced and sealed.
...
```text
1. Continue Phase 17 after the sealed session, debug, connector/egress and three prompt-boundary slices by inventorying remaining model-input/output crossings plus native-skill/tool provenance and entitlement enforcement. Select the smallest concrete enforceable gap from repository evidence; reuse canonical identity, authority, permission, provenance and audit owners rather than creating parallel security truth.
...
5. Complete risk-tiered Phase 17 secure-tool, remaining prompt-injection, skill/tool supply-chain, provenance, broader adversarial-regression and incident-response controls before materially expanding external or consequential autonomous execution.
