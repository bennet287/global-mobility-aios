...
**Current checkpoint:** The Phase 16 timeout/cancellation tranche is sealed through PR #176. Phase 16.3A's provider-attempt ledger is sealed by PR #179; Phase 16.3B extends it across direct model calls by PR #181. Phase 16.3C adds opt-in provider call-count capacity by PR #183. Phase 16.3D adds optional per-request DeepSeek/Moonshot output-token caps by PR #185. Phase 16.3E adds read-only cost-evidence visibility by PR #187. Phase 16.3F preserves optional provider completion IDs and inventories identified API external calls by PR #189 at merge `67a667181fbdb8b875fd6557589bb94649013f76`. Source-linked billed cost, verified paid-tool charges, Board USD allocation, a provable pre-call monetary ceiling, and Gemini output-cap verification remain open. System-1/AX evaluation remains recorded by PR #167.
**Code migration head:** `0088_provider_response_identity`

<!-- CURRENT_MIGRATION_HEAD: 0088_provider_response_identity -->
...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; the provider-call attempt ledger is sealed for controlled AgentRuns by PR #179 and for every direct model-call path by PR #181; opt-in admin-governed cumulative provider call capacity is sealed by PR #183; optional DeepSeek/Moonshot per-call output-token caps are sealed by PR #185; read-only cost-evidence visibility is sealed by PR #187; and provider completion identity plus the identified API external-call inventory are sealed by PR #189. The next bounded runtime work requires authoritative itemized provider billing evidence and a verified per-attempt join, plus real vendor cost contracts for paid tools before monetary allocation. Gemini output-cap compatibility remains separately unverified; scoped circuit breakers and reconciliation follow. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
...
```text
1. Continue Phase 16 by obtaining an authoritative itemized billing source and proving a unique per-attempt match; determine the real vendor charges for identified paid-tool boundaries before governed monetary allocation. Verify Gemini's per-call output-cap contract separately; DeepSeek/Moonshot output caps are optional and sealed. Preserve the distinction between authorized monetary allocation, opt-in call-count capacity, observed actual spend and estimates/unattributed cost. Do not claim a hard monetary budget without authoritative cost evidence and a provable pre-call ceiling.
2. Follow with scoped circuit breakers and runtime reconciliation before materially increasing autonomous execution.
