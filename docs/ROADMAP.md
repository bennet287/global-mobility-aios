...
**Current checkpoint:** The Phase 16 timeout/cancellation tranche is sealed through PR #176. Phase 16.3A's provider-attempt ledger is sealed by PR #179; Phase 16.3B extends it across direct model calls by PR #181. Phase 16.3C adds opt-in provider call-count capacity by PR #183. Phase 16.3D adds optional per-request DeepSeek/Moonshot output-token caps by PR #185 at merge `15735f3815d007390b1c5c87530881527450ae19`. Governed monetary allocation prerequisites, authoritative billed cost, paid-tool coverage, and Gemini output-cap verification remain open. System-1/AX evaluation remains recorded by PR #167.
...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; the provider-call attempt ledger is sealed for controlled AgentRuns by PR #179 and for every direct model-call path by PR #181; opt-in admin-governed cumulative provider call capacity is sealed by PR #183; and optional DeepSeek/Moonshot per-call output-token caps are sealed by PR #185. The next bounded runtime work is monetary allocation prerequisites and paid-tool/billing reconciliation, with Gemini output-cap compatibility separately verified, followed by scoped circuit breakers and reconciliation. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
