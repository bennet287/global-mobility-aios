...
Phase 17A–17G now form the accepted security-assurance baseline. PR #198 bounds signed application-session lifetime. PR #200 restricts `/debug` payloads to admins. PR #203 restricts connector creation, status changes, and active health checks to admins, while PR #205 separately enforces the webhook destination/connection-time address boundary: HTTPS only, no embedded credentials, no mixed/non-public resolved destinations, pinned connection to a prevalidated public address with original Host/SNI identity, and no environment proxies or redirects. PR #206 separates regulatory-classifier policy/output contract from lower-trust official-source evidence; PR #207 applies the same explicit trust separation to business advisory; PR #208 applies it to controlled agents, where the system prompt retains role-card instructions plus the canonical output schema and the user envelope is limited to `operator_task` plus `untrusted_context`. PR #210 separates the in-house consultant's trusted role-card, controlled-agent inventory, and output contract from lower-trust operator message, conversation, lead, and UI-hint evidence, with adversarial provider-boundary coverage. These slices reuse existing auth, connector, provider, deterministic-validation, audit, and accounting owners; they do not create a parallel security/authority state or grant new tool authority.

The next Phase 17 work is evidence-led inspection of an executable agent-to-tool seam and remaining native-skill/tool supply-chain, provenance, adversarial-regression, and incident-response gaps. The existing `ContextBundle.allowed_tools` is derived from governed position/task context, then intersected with `AgentRuntimeProfile.available_tools` in a fresh runtime binding. In the inspected paths, the Austria live provider profile exposes no tools, controlled-agent provider calls offer no tool-dispatch API, native-skill matching is diagnostic and excludes unresolved tool prerequisites, and automation delivery applies distinct human-review and connector gates. No current agent `tool_id`/action invocation seam consumes the binding allowance. When a real seam exists, enforce current canonical membership there with a focused denial regression, preserving the existing command and external-action authority rules. Do not create a speculative Phase 17H control or claim that the binding alone governs independent command/connector execution. The CodeQL workflow added by PR #202 remains useful scan evidence, but a successful scan does not itself seal runtime trust boundaries.
...
```text
1. Continue Phase 17 after sealed Phase 17A–17G by inspecting concrete agent tool-invocation and native-skill/tool provenance paths. Use the existing governed context/runtime allowance only at an actual executable agent-to-tool seam, with a current-binding membership check and denial proof; preserve separate command, human-review, and connector gates. Continue evidence-led adversarial and incident-response work without creating parallel security truth or a speculative Phase 17H layer.
...
5. Complete risk-tiered Phase 17 secure-tool, prompt-injection, skill/tool supply-chain, provenance, adversarial-regression and incident-response controls before materially expanding external or consequential autonomous execution.
