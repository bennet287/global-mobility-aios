...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; and explicit AgentRun cancellation is sealed by PR #176. The timeout/cancellation tranche is therefore complete. The next bounded runtime control is hard runtime budgets / actual cost metering, followed by scoped circuit breakers and reconciliation. These controls must continue to reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
...
```text
1. Continue Phase 16 with hard runtime budgets / actual cost metering, preserving the distinction between authorized allocation, observed actual spend and estimates/unattributed cost.
