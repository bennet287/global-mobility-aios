...
```text
1. Continue Phase 16 with scoped provider circuit breakers before materially increasing autonomous execution. AgentRun-linked stranded provider attempts are reconciled only after the run finishes and the hard-limit grace period passes; direct request-local attempts still lack an execution-end signal. Use classified transport failures and the existing attempt ledger; preserve unknown spend for interrupted calls.
