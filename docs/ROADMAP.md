...
Phase 17A–17F now form the accepted security-assurance baseline. PR #198 bounds signed application-session lifetime. PR #200 restricts `/debug` payloads to admins. PR #203 restricts connector creation, status changes, and active health checks to admins, while PR #205 separately enforces the webhook destination/connection-time address boundary: HTTPS only, no embedded credentials, no mixed/non-public resolved destinations, pinned connection to a prevalidated public address with original Host/SNI identity, and no environment proxies or redirects. PR #206 separates regulatory-classifier policy/output contract from lower-trust official-source evidence; PR #207 applies the same explicit trust separation to business advisory; PR #208 applies it to controlled agents, where the system prompt retains role-card instructions plus the canonical output schema and the user envelope is limited to `operator_task` plus `untrusted_context`. These slices reuse existing auth, connector, provider, deterministic-validation, audit, and accounting owners; they do not create a parallel security/authority state or grant new tool authority.

The next bounded Phase 17 model-input crossing is the existing in-house consultant path. Its system prompt already owns the role-card instructions and output schema, while the operator message, recent conversation history, lead records, and UI lead hint are concatenated into the provider user message. Phase 17G should make those values explicitly lower-trust evidence and add a provider-boundary adversarial regression, while preserving deterministic fallback, controlled-agent registry validation, lead validation, provider accounting, and the accepted request-owner completion evidence. After that bounded slice, continue evidence-led inspection of the remaining skill/tool supply-chain, provenance, adversarial-regression, and incident-response gaps rather than introducing a generic security framework by assumption. The CodeQL workflow added by PR #202 remains useful scan evidence, but a successful scan does not itself seal runtime trust boundaries.
...
```text
1. Continue Phase 17 after sealed Phase 17A–17F by hardening the existing in-house consultant provider boundary identified in current code. Keep role-card instructions and the output contract trusted; treat the operator message, recent conversation, lead records and UI lead hint as lower-trust evidence; add a provider-boundary adversarial regression; preserve deterministic fallback, registry/lead validation, provider accounting and request-owner completion evidence. Then continue evidence-led inspection of the remaining skill/tool supply-chain, provenance, adversarial-regression and incident-response gaps without creating parallel security truth.
