...
**Current checkpoint:** The Phase 16 timeout/cancellation tranche is sealed through PR #176. Phase 16.3A's provider-attempt ledger is sealed by PR #179; Phase 16.3B extends it across direct model calls by PR #181. Phase 16.3C adds opt-in provider call-count capacity by PR #183. Phase 16.3D adds optional per-request DeepSeek/Moonshot output-token caps by PR #185. Phase 16.3E adds read-only cost-evidence visibility by PR #187 at merge `c2cf9614742f2d1338761946380f50a58756a39e`. Source-linked billed cost, paid-tool coverage, Board USD allocation, a provable pre-call monetary ceiling, and Gemini output-cap verification remain open. System-1/AX evaluation remains recorded by PR #167.
...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; the provider-call attempt ledger is sealed for controlled AgentRuns by PR #179 and for every direct model-call path by PR #181; opt-in admin-governed cumulative provider call capacity is sealed by PR #183; optional DeepSeek/Moonshot per-call output-token caps are sealed by PR #185; and read-only cost-evidence visibility is sealed by PR #187. The next bounded runtime work is source-linked billing reconciliation and paid-tool cost inventory before monetary allocation, with Gemini output-cap compatibility separately verified, followed by scoped circuit breakers and reconciliation. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
