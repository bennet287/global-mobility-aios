...
**Current checkpoint:** The Phase 16 timeout/cancellation tranche is sealed through PR #176. Phase 16.3A's controlled AgentRun provider-attempt ledger is sealed by PR #179; Phase 16.3B extends that ledger to every direct model-call path by PR #181 at merge `258678f0ba13af2401d5039f8d5c5e6d97a56d2d`. Governed runtime allocation and enforceable per-call limits remain next; authoritative billed cost and paid-tool coverage remain open. System-1/AX evaluation remains recorded by PR #167.
...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; and the provider-call attempt ledger is sealed for controlled AgentRuns by PR #179 and for every direct model-call path by PR #181. The next bounded runtime control is governed allocation and enforceable per-call limits, after accounting for paid tools and uncertain billing evidence, followed by scoped circuit breakers and reconciliation. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
...
```text
1. Continue Phase 16 with governed runtime allocation and enforceable provider-specific pre-call limits. Reconcile paid tools beyond direct model calls and the authority/billing boundary; preserve the distinction between authorized allocation, observed actual spend and estimates/unattributed cost. Do not claim a hard monetary budget without authoritative cost evidence and a provable ceiling.
