...
**Current checkpoint:** The Phase 16 timeout/cancellation tranche is sealed through PR #176. Phase 16.3A's controlled AgentRun provider-attempt ledger is sealed by PR #179; Phase 16.3B extends that ledger to every direct model-call path by PR #181. Phase 16.3C adds opt-in, admin-governed per-provider call-count capacity by PR #183 at merge `c4ff79ff189d7f4a1521e226365cacc9b1c9f5e8`. Provider-specific per-call output limits and prerequisites for governed monetary allocation remain next; authoritative billed cost and paid-tool coverage remain open. System-1/AX evaluation remains recorded by PR #167.
...
```text
1. Continue Phase 16 with provider-specific pre-call output limits and the prerequisites for governed monetary allocation. Reconcile paid tools beyond direct model calls and the authority/billing boundary; preserve the distinction between authorized monetary allocation, the already sealed opt-in call-count capacity, observed actual spend and estimates/unattributed cost. Do not claim a hard monetary budget without authoritative cost evidence and a provable ceiling.
