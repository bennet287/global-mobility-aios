...
Current runtime reliability foundation: provider-usage observations are sealed by PR #165; deterministic failure classification and transport-only retry are sealed by PR #166; cooperative soft-timeout handling is sealed by PR #171; stale-running reconciliation is sealed by PR #173; explicit AgentRun cancellation is sealed by PR #176; and the controlled AgentRun provider-attempt ledger is sealed by PR #179. The next bounded runtime control is governed allocation and hard per-call budget enforcement, after accounting for other paid paths and uncertain billing evidence, followed by scoped circuit breakers and reconciliation. These controls must reuse canonical runtime/evidence truth rather than create a second execution-state store merely for signals.
...
```text
1. Continue Phase 16 with governed allocation and hard per-call budget enforcement after reconciling paid paths outside the controlled-AgentRun ledger; preserve the distinction between authorized allocation, observed actual spend and estimates/unattributed cost.
