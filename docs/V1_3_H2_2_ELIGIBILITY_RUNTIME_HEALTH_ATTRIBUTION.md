...
H.2.2 is not accepted because the code exists. The candidate must prove at least:
...
7. repeated runtime-health failures remain observation-only and do not open a circuit;
8. H.2.1 verifier-disagreement recurrence remains unchanged;
9. broad SQLite regression remains green;
10. the H.2.2 tests run in the real PostgreSQL governance lane;
11. migration/schema, repository policy, dependency constraints and frontend proof remain green through V12 Production Proof.
