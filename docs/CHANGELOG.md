## 2026-08-17 - Phase 13.16.7 COMPLETE / PASS - Secure Mobility User plan and evidence experience

- Upgraded the existing token/device-bound `/portal` experience instead of creating a new public case dashboard; `/my-mobility` remains a non-sensitive orientation/access surface.
- Extended the existing `GET /api/v1/public/client-portal/dashboard` response with nullable `mobility_plan` and `evidence_summary` projections only. No new router or public endpoint was added.
- `mobility_plan` selects only an active/completed timeline with explicit human activation, exact lead/profile/comparison/pathway/version pinning, current granted consent, non-simulation comparison state, and a published/superseded/retired pathway version carrying approval/publication provenance. Newer draft, simulation, stale, unactivated, or mismatched records are skipped instead of leaked or recomputed.
- The client plan exposes route identity, plan state, pathway version, activation/publication dates, processing-evidence state, government application fee/scope, minimum-funds threshold, estimated-total evidence status, risk level/category counts, and client-normalized durable milestones. It intentionally excludes raw verified-rule/source IDs, hashes, review notes, blockers, operator ownership, internal evidence arrays, publication controls, and timeline mutation controls.
- `evidence_summary` exposes only the newest approved `DocumentRequirementAssessment` aligned to the selected pathway/version and current profile/version, limited to result/review state and required/satisfied/missing/inconsistency counts. Pending/rejected assessments, reviewer identity/notes, findings, source snapshots, and document snapshots remain internal.
- Generic `FollowUp` / communications records are intentionally not exposed because the current model has no universal client-visible/reviewed classification. Country-ranking PR/citizenship outputs are also not promoted into this first portal slice; long-term progression comes from the already human-activated durable timeline.
- Added focused API regressions for client-safe reviewed-plan projection, skipping newer simulation/draft state, approved-evidence selection, and stale-profile suppression; added frontend type coverage, premium journey/cost/risk/evidence composition, truthful no-plan state, and design-foundation safety assertions.
- Browser runtime acceptance exposed a CORS transport gap: the browser correctly sends `X-GMAI-Portal-Token` and `X-GMAI-Portal-Device`, but the API allow-list did not admit them. The explicit CORS allow-list now includes both portal headers and a preflight regression protects the browser contract without weakening token hashing, expiry, lead scope, device binding, audit behavior, or backend authorization.
- **Final acceptance:** client-portal/CORS regression PASS; design-foundation **24/24 PASS**; Next.js 15.2.4 production build **41/41 PASS**; complete API regression **811 passed / 5 skipped / 0 failed** with only the known Starlette/httpx test-client deprecation warning.
- Repository policy, release consistency, migration consistency, local physical-schema, exact ten-file boundary, UTF-8/presentation, and diff gates PASS at Alembic `0076_organization_position_active_identity`, with 118 registered, 118 actual model tables, and 119 physical tables including only `alembic_version` infrastructure.
- Disposable runtime/browser acceptance PASS for both governed states: a human-activated reviewed plan with conservative cost/risk/evidence/journey composition, and an honest no-plan state that exposes no fabricated pathway, fee, risk, evidence conclusion, or long-term plan. The preserved `gmai.db` remained unchanged.
- **13.16.7 is COMPLETE / PASS. 13.16.8 Professional / Operator experience is UNLOCKED / NEXT.**
## 2026-08-17 - Phase 13.16.6 COMPLETE / PASS - Owner decision and escalation inbox

- Added `/owner-inbox` as a dedicated Owner/Board attention-triage surface. It is intentionally distinct from Board Room: Owner Inbox determines what genuinely requires Owner attention and routes to the legitimate authority/operational surface; Board Room continues to execute reserved Board decisions and organization control.
- Composed only existing reads: Board Packet, durable HumanActionRequests, WorkItems, open Blockers, active Dependencies, and durable Activity. No backend endpoint, model, migration, or new authority command was added.
- Added four deterministic authority/materiality tiers: **Decision required** (`pending_board` only); **Critical Owner attention** (critical/high Board-attention risks, emergency work, critical Owner-relevant human requests, critical blockers on Board-owned/accountable work); **Human / escalation required** (explicitly Owner-relevant active human requests, overdue Board-owned work, unresolved dependencies blocking Board-owned downstream work); and **Watch** (lower-severity Board-attention risk plus elevated-risk Board-owned context without overdue/emergency state).
- Frozen the promotion rule that **HumanActionRequest exists ≠ Owner attention**. The generic authentication role `admin` does not itself establish Board authority; the canonical Human Board position key `board` does. Promotion requires explicit Board-position, pending-Board-decision, Board-owned-work, or Board-accountable-blocker evidence. No title/keyword scoring, LLM classification, or probabilistic priority is used.
- Each item explains why it was promoted, affected work/department context, authority, timing, and latest durable Activity when available, then links to Board Room, cross-department friction, or the relevant department workspace. The page performs no Board decision/control, blocker/dependency/work, publication/certification, or legal-state mutation.
- Added Owner navigation and Cockpit deep links to `/owner-inbox`, plus premium responsive styling and a design-foundation regression that verifies read bindings, deterministic authority signals, routing, and absence of mutation commands.
- **Final acceptance:** design-foundation **23/23 PASS**; organization read-client **2/2 PASS**; Next.js 15.2.4 production build **41/41 pages** including `/owner-inbox`; complete API regression **807 passed / 5 skipped / 0 failed** with only the known Starlette/httpx test-client deprecation warning; repository policy, release consistency, migration, local physical-schema, UTF-8, exact seven-file boundary, and diff checks **PASS** at Alembic `0076_organization_position_active_identity`. Runtime/browser acceptance reached **Workspace Ready** with no partial-data warning, truthful zero states, clear four-tier authority/materiality hierarchy, delegated human work separated from Owner authority, governed routing, and no duplicated mutation controls. Final secondary-action contrast polish passed design-foundation and production build. No backend endpoint, model, migration, preserved database, authorization expansion, publication/certification, human-review, organization-control, or Austria legal-safety state changed. **13.16.6 is COMPLETE / PASS; 13.16.7 Mobility User experience is UNLOCKED / NEXT.**
...
- Added a dedicated Owner surface `/cross-department-friction` (`apps/web/app/cross-department-friction/page.tsx`) that loads organization-wide open blockers, active dependencies, work items, pending human requests, and material Activity. It surfaces blockers whose owning department differs from the affected work's department and dependencies whose downstream/upstream work items belong to different departments, answering: what is blocked, why it is blocked, which department owns the underlying work, which dependency is unresolved, whether human action is required, whether escalation/overdue has occurred, and what changed materially.
- Each friction row shows involved departments, accountable position, due/overdue state, an active human-request indicator, an escalation/overdue flag, the newest durable Activity, deep links into each affected `/workspace/[department]`, and a governed `OrganizationHumanActionRequest` intervention trigger. The page does not resolve/waive blockers, satisfy/waive dependencies, complete/reassign work, make Board decisions, publish/certify evidence, issue legal conclusions, or alter organization control.
- Added navigation support in `apps/web/lib/workspace-navigation.ts`: a "Cross-department friction" item under the Owner "Department workspaces" group and explicit Owner experience mapping for `/cross-department-friction`.
- Added an Owner-control-dock link in `apps/web/app/cockpit/page.tsx` from the Cockpit to `/cross-department-friction`.
- Added Phase 13.16.5 styling in `apps/web/app/globals.css` (`.cross-department-friction`, `.friction-summary`, `.friction-grid`, `.friction-lane`, `.friction-list`, `.friction-row-*`, severity/attention/activity tokens, responsive and dark-mode variants) plus a `.premium-button.small` utility.
- Added design-foundation regression assertions in `apps/web/scripts/design-foundation.test.mjs` for the new route, nav mapping, Cockpit link, list function bindings, UI classes, human-action/escalation indicators, and absence of direct mutation commands.
- No backend model, service, router, Alembic migration, preserved PostgreSQL mutation, or Austria safety-boundary change was introduced. The existing organization read endpoints continue to provide all data.
- **Acceptance:** design-foundation **22/22 PASS**; organization read-client **2/2 PASS** (unchanged from 13.16.4); Next.js 15.2.4 production build **PASS** with **40/40 pages** including `/cross-department-friction`; repository policy, database migration, and local DB schema checks **PASS** at Alembic `0076_organization_position_active_identity`; `git diff --check` **PASS**; complete API regression **806 passed, 5 skipped, 0 failed**; runtime smoke **8/8 HTTP 200** across `/health`, Board Packet, Observatory departments, blockers, dependencies, work items, human-action requests, and contributions.
- **State:** **13.16.5 COMPLETE / PASS. 13.16.6 Owner decision and escalation inbox is UNLOCKED / NOT STARTED.**
