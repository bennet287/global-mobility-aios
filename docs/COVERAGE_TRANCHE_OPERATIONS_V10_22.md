...
## v10.22.1 path-resolution correction

The v10.22.1 hotfix resolves relative manifest and output paths through PowerShell's active provider location rather than the process working directory used by `System.IO.Path.GetFullPath`. This matters when the PowerShell prompt has changed directories with `Set-Location` but the host process working directory still points at the parent folder.

Relative examples such as `./knowledge/global_coverage/tranches/v10_22_operations_manifest.example.json` now resolve from the visible PowerShell location. Absolute paths continue to work unchanged. The correction changes scripts and documentation only; it does not call an API, mutate data, or alter the existing safety boundaries.
