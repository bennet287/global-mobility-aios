...
Write-Host 'Controlled coverage source canonical-URL remediation'
...
with Session(engine) as session:
    ...
    parsed_old = urlparse(source.url)
    ...
    record_audit(
        ...
        actor=actor,
        source="coverage_source_remediation_v10_18_1",
    )
    session.commit()
...
print(json.dumps({
    ...
    "source_monitor_id": str(monitor.id),
    "old_url": old_url,
    "new_url": new_url,
    "monitor_status": monitor.status,
