...
def _item_state(session: Session, item: JurisdictionCoverageEvidenceBatchItem) -> dict[str, Any]:
    assessment = (
        session.get(JurisdictionImmigrationAssessment, item.immigration_assessment_id)
    ...
    assessment_approved = bool(
        ...
        and (assessment.official_source_id is None or assessment.official_source_id == item.official_source_id)
    )
