...
export default function GlobalIntelligencePage() {
  ...
  const [coverageBatchJson, setCoverageBatchJson] = useState(`[
  {
    ...
    "immigration_assessment": {
      ...
      "rationale": "Official evidence identifies the authority that administers immigration rules."
    }
  }
]`);
  ...
  async function publishReviewedInitialRule(assertionId: string) {
    setInitialAssertionBusy(assertionId); setError(null);
    try {
      await publishInitialRuleAssertion(assertionId, (initialAssertionPublishNotes[assertionId] || "").trim());
            ...
            <div className="coverage-initial-rules">
              ...
              <div className="coverage-initial-rule-grid">
                ...
                <div className="coverage-initial-rule-list">
                  {initialRuleAssertions.length ? initialRuleAssertions.map((assertion) => <article key={assertion.id}><div><div><span>{assertion.alpha2_code || "—"} · {titleCase(assertion.domain)}</span><h4>{assertion.title}</h4></div><StatusBadge value={assertion.status} /></div><p>{assertion.statement}</p><blockquote>{assertion.evidence_excerpt}</blockquote><small>{Math.round(assertion.confidence * 100)}% confidence · snapshot {assertion.snapshot?.id.slice(0, 8) || "missing"} · proposed by {assertion.proposed_by}</small>{assertion.status === "pending_review" && <div className="coverage-initial-rule-action"><textarea placeholder="Independent review notes" value={initialAssertionReviewNotes[assertion.id] || ""} onChange={(event) => setInitialAssertionReviewNotes((current) => ({ ...current, [assertion.id]: event.target.value }))} /><div><button className="button primary" disabled={initialAssertionBusy === assertion.id || (initialAssertionReviewNotes[assertion.id] || "").trim().length < 3} onClick={() => void decideInitialRuleAssertion(assertion.id, "approved")}>Approve</button><button className="button danger" disabled={initialAssertionBusy === assertion.id || (initialAssertionReviewNotes[assertion.id] || "").trim().length < 3} onClick={() => void decideInitialRuleAssertion(assertion.id, "rejected")}>Reject</button></div></div>}{assertion.status === "approved" && <div className="coverage-initial-rule-action"><textarea placeholder="Publication notes and attestation basis" value={initialAssertionPublishNotes[assertion.id] || ""} onChange={(event) => setInitialAssertionPublishNotes((current) => ({ ...current, [assertion.id]: event.target.value }))} /><button className="button primary" disabled={initialAssertionBusy === assertion.id || (initialAssertionPublishNotes[assertion.id] || "").trim().length < 3} onClick={() => void publishReviewedInitialRule(assertion.id)}>Publish verified rule</button></div>}{assertion.status === "published" && <small>Published rule {assertion.verified_rule?.id.slice(0, 8)} · no source change event created.</small>}</article>) : <EmptyState title="No initial rule assertions" detail="An independently approved jurisdiction with an immutable baseline snapshot can be submitted for review here." />}
                </div>
              </div>
            </div>
