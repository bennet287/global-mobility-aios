...
export default function DocumentIntelligencePage() {
  ...
  async function chooseLead(id: string) {
    setLeadId(id); setDocuments([]); setJobs([]); setValidations([]); setExpiryReminders([]); setRequirementAssessments([]); setError(null); setMessage(null);
    if (!id) return;
    setLoading(true);
    try {
      const [detail, jobRows, validationRows, reminderRows, requirementRows] = await Promise.all([
        getLeadDetail(id),
      ...
      setDocuments(detail.documents); setJobs(jobRows); setValidations(validationRows); setExpiryReminders(reminderRows); setRequirementAssessments(requirementRows);
  ...
  async function refreshJobs() {
    ...
    setWorking("refresh");
    try {
      const [jobRows, validationRows, reminderRows, requirementRows] = await Promise.all([
      ...
      setJobs(jobRows); setValidations(validationRows); setExpiryReminders(reminderRows); setRequirementAssessments(requirementRows);
    }
