...
export default function DocumentIntelligencePage() {
  ...
  async function chooseLead(id: string) {
    setLeadId(id); setDocuments([]); setJobs([]); setValidations([]); setExpiryReminders([]); setError(null); setMessage(null);
    if (!id) return;
    setLoading(true);
    try {
      const [detail, jobRows, validationRows, reminderRows] = await Promise.all([
        getLeadDetail(id),
        listDocumentExtractions({ lead_id: id }),
        listDocumentConsistencyAssessments(id),
        listDocumentExpiryReminders({ lead_id: id }),
      ]);
      setDocuments(detail.documents); setJobs(jobRows); setValidations(validationRows); setExpiryReminders(reminderRows);
  ...
  async function refreshJobs() {
    ...
    setWorking("refresh");
    try {
      const [jobRows, validationRows, reminderRows] = await Promise.all([
      ...
      setJobs(jobRows); setValidations(validationRows); setExpiryReminders(reminderRows);
    }
