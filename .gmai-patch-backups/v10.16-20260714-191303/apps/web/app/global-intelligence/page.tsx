...
export default function GlobalIntelligencePage() {
  ...
  }, [authorityFilter, confidenceFilter, coverageFilter, coverageWorkGap, coverageWorkRegion, freshnessFilter, materialityFilter, reviewStateFilter, windowDays]);
      ...
      {dashboard && <>
        ...
        {tab === "coverage" && <div className="global-registry-stack">
          ...
          <section className="panel coverage-operations-panel">
            <SectionTitle label="Coverage operations" title="Evidence worklist and controlled batch submission" detail="Prioritize remaining jurisdictions and submit evidence proposals without bypassing separate human review" />
            ...
            <div className="coverage-operations-grid">
              ...
              <div className="coverage-batch-card">
                <div className="coverage-card-heading"><div><span>Controlled evidence package</span><h3>Submit up to 50 jurisdictions</h3></div><StatusBadge value="human_review_required" /></div>
                ...
                <small>Batch submission is atomic and idempotent. It creates pending proposals only; a different authenticated reviewer must approve each assessment and certification.</small>
              </div>
            </div>
            <div className="coverage-batch-history">
              ...
              {coverageBatches.length ? <div className="coverage-batch-list">{coverageBatches.slice(0, 8).map((batch) => <article key={batch.id}><div><strong>{batch.name}</strong><StatusBadge value={batch.status} /></div><p>{batch.notes}</p><small>{batch.item_count} jurisdictions · {batch.immigration_assessment_count} assessments · {batch.source_certification_count} certifications · submitted by {batch.submitted_by}</small><div><span>{batch.review_counts.pending_review || 0} pending</span><span>{batch.review_counts.approved || 0} approved</span><span>{batch.review_counts.rejected || 0} rejected</span></div></article>)}</div> : <EmptyState title="No evidence batches" detail="Use the controlled evidence package to create the first immutable batch." />}
            </div>
