            ...
            <div className="coverage-tranche-assistant">
              ...
              {trancheAssistantResult && <div className="coverage-tranche-results">
                ...
                {trancheAssistantResult.items.map((item) => <article key={item.batch_item_id}>
                  ...
                  {item.candidate_assertion ? <div className="coverage-tranche-draft"><strong>Constrained draft suggestion</strong><p>{item.candidate_assertion.statement}</p><blockquote>{item.candidate_assertion.evidence_excerpt}</blockquote><small>Human editing and independent review remain mandatory. No assertion has been created.</small><button className="button secondary" onClick={() => useAssistantDraft(item)}>Copy draft into assertion form</button></div> : item.snapshot_analysis && <InlineNotice label="No safe draft generated" detail="The snapshot is missing substantive immigration text or is too navigation-heavy. Select a better official page and capture a new approved baseline instead of forcing a rule." tone="warn" />}
                </article>)}
              </div>}
            </div>
