...
## Delivery Status — v10.9 (2026-07-14)

Current database migration head: `0025_document_fraud_risk_assessments`.
...
- Phase 9 has completed extraction, profile/application validation, expiry reminders,
  missing-document detection, broader inconsistency detection, and human-reviewed
...
- The immediate remaining Phase 9 release is signed, expiring document access and
  production object-storage controls.
...
1. Complete signed, expiring document access and production object-storage controls.
2. Add the remaining Global Intelligence dashboard filters.
3. Complete reviewed immigration-rule relationships and primary authority/source coverage.
4. Build global country ranking and country-level trade-off explanations only after the
   reviewed catalogue and coverage gates are sufficiently complete.
5. Extend the timeline engine into immutable multi-year and multi-country scenarios.

### Remaining-work ledger

- **Phase 9:** one capability group remains.
- **Phase 10B:** two global-coverage completion items remain.
- **Phase 10C:** one dashboard-filter item remains.
- **Phase 10D:** three global user-intelligence items remain.
...
- [ ] Signed, expiring document access and production object-storage controls
  - [ ] Issue short-lived, audited access grants instead of durable or direct object URLs
  - [ ] Enforce authenticated role, lead ownership, document scope, and purpose on every grant
  - [ ] Support local storage and MinIO without exposing storage credentials or unrestricted object keys
  - [ ] Record grant creation, access, expiry, revocation, denial, and actor provenance
  - [ ] Add explicit revocation and fail-closed handling for expired, altered, missing, or unauthorized objects
  - [ ] Add production bucket privacy, TLS, encryption, retention/lifecycle, backup, and recovery guidance
  - [ ] Add restricted operator controls and regression coverage without changing document verification state
...
- [ ] Freshness, coverage, authority, confidence, materiality, and review-state filters
...
- [ ] Rank best-fit countries across the complete reviewed global catalogue
- [ ] Explain country-level tradeoffs, long-term goals, PR/citizenship dependencies, and uncertainty
- [ ] Reassess only when the user explicitly accepts a new profile or reviewed regulatory version
