...
function ChangeList({ items, empty }: { items: GlobalIntelligenceChange[]; empty: string }) {
  return <div className="global-change-list">{items.length ? items.map((item) => <article key={item.id}>
    ...
    <p>{item.summary}</p>
    <div className="global-change-meta"><span>{titleCase(item.domain)}</span><span>{titleCase(item.materiality)}</span><span>{formatDate(item.detected_at)}</span></div>
    <small>{item.source_name || "Source unavailable"}{item.reviewed_by ? ` · reviewed by ${item.reviewed_by}` : " · awaiting review"}</small>
...
export default function GlobalIntelligencePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [healthResult, intelligence, registryResult, authorityResult, sourceResult] = await Promise.all([
        getHealthStatus(), getGlobalIntelligenceDashboard(windowDays), getGlobalJurisdictionRegistry(),
