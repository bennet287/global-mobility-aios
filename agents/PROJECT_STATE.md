...
**Active implementation:** none; Phase 17A session expiry and Phase 17B debug-route access are sealed by PR #198 and PR #200. Phase 16 itemized billing/vendor-cost attribution, Board USD allocation, a provable pre-call monetary ceiling, and Gemini output-cap verification remain open prerequisites rather than claimed capabilities.
**Next scheduled slice:** inspect real external-tool invocation and untrusted-content crossings, then select the smallest enforceable Phase 17 control justified by repository evidence. Reuse existing identity, authority, permission and audit boundaries.
...
Phase 17A/17B bounded session replay lifetime and restricted debug-route payloads using existing auth policy. Neither completes broader secure-tool, prompt-injection, skill/tool supply-chain, provenance, adversarial-regression or incident-response work. Inspect actual external-tool invocation and untrusted-content crossings before selecting the next control; preserve existing public client/partner APIs and canonical authority boundaries.
