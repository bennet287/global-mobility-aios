...
**Active implementation:** none; Phase 17A bounded authenticated-session lifetime is sealed by PR #198. Phase 16 itemized billing/vendor-cost attribution, Board USD allocation, a provable pre-call monetary ceiling, and Gemini output-cap verification remain open prerequisites rather than claimed capabilities.
**Next scheduled slice:** inspect the existing public `/debug` authentication boundary and the concrete debug-route payloads, then select the smallest risk-tiered Phase 17 exposure control justified by repository evidence. Do not create parallel identity, authority, permission, or security truth.
...
Phase 17A closed one concrete authentication gap without adding a new security subsystem: signed sessions now have bounded temporal validity. The next security slice must remain evidence-led. Inspect why `/debug` is currently part of the public-path policy, enumerate the actual debug payloads and production exposure, and then apply only the smallest justified control. Do not silently break intended public client/partner APIs, move authorization into a second store, or treat debug-route hardening as completion of broader secure-tool, prompt-injection, skill/tool supply-chain, provenance, adversarial-regression or incident-response work.
