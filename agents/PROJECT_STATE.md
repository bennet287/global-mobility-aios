...
**Active implementation:** none; the controlled AgentRun provider-attempt ledger is sealed
**Next scheduled slice:** governed allocation and hard per-call budget enforcement, after reconciling all paid execution paths and billing evidence
...
- Phase 16.3A Runtime Economics Ledger — SEALED by PR #179 at merge `b42a0126aa5ee7c6e9733c1a6dd9240321af08ae`. Controlled AgentRun provider attempts are durably recorded before the paid call and settled with observed usage or unknown outcome. Estimates remain non-billing evidence. This covers the controlled-agent path only; allocation, hard budgets, and other paid execution paths remain open.
...
- Phase 16.3A `AgentRunProviderAttempt` is paid-call accounting evidence for controlled AgentRuns, keyed by run and attempt. It does not grant budget or represent invoices; `billed_cost_usd` remains unknown without authoritative provider evidence.
...
Before implementing budget enforcement, reconcile Phase 16.3A's controlled-agent ledger with every other paid execution path, existing allocation/budget concepts, retry/cancellation interactions, and the exact authority boundary for granting or replenishing runtime spend. Do not imply that paths lacking an AgentRun are covered by the ledger.
