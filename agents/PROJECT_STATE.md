...
**Active implementation:** none; the Phase 16 runtime reliability foundation is sealed through PR #196, while itemized billing/vendor-cost attribution, Board USD allocation, a provable pre-call monetary ceiling, and Gemini output-cap verification remain open prerequisites rather than claimed capabilities.
**Next scheduled slice:** begin bounded Phase 17 security assurance from the existing identity/authorization, tool-invocation, skill, provenance, connector and audit boundaries; inspect current controls before selecting the smallest enforceable gap. Phase 16 monetary-evidence prerequisites remain separate and must not be papered over with estimates.
...
The next active programme is Phase 17 security assurance. Start by inspecting the actual existing identity/authorization, permission, connector/tool invocation, native-skill, provenance and audit boundaries and threat-model the real trust crossings before adding state. Reuse canonical controls where they already exist; security assurance must not become a parallel authorization store, a blanket pre-tool database write, or a justification for broader authority/autonomy. Controls should be proportionate to consequence, with the strongest enforcement at credential, external-mutation and other consequential boundaries.
