...
**Active implementation:** none; AgentRun-linked attempt reconciliation, provider circuit admission, and one real request-local completion boundary are sealed through PR #194
**Next scheduled slice:** extend explicit request-local execution-end evidence only to another real owner boundary, starting business advisory; then progress Phase 17 security assurance. Itemized billing joins and vendor cost attribution remain prerequisites for governed USD allocation; verify Gemini's output-cap contract separately.
...
- Phase 16.4C Request-local Provider Completion Evidence — SEALED by PR #194 at merge `08e0e0786d6201854ade054d04ddb11fc26ad320`. The in-house consultant now supplies a stable request operation key and records durable owner-completion evidence through the existing `AuditLog` boundary. The reconciler may close a still-`started` request-local attempt to `outcome_unknown` only after that explicit owner signal. Age alone is insufficient; call slots stay consumed; no provider failure, charge, refund, zero usage, or completed external effect is inferred. Business advisory remains outside this owner hook until separately wired.
...
- Phase 16.4C reuses `ProviderCallAttempt.operation_key` plus existing `AuditLog` evidence for real request-local owner completion. Only explicitly wired owners may produce that signal; it is not a generic timeout and does not settle billing truth.
...
## Next slice guardrails — remaining request-local completion and monetary budget prerequisites

The enrolled-provider circuit uses classified transport failures rather than inferring failure from an unknown billing outcome. It stays open until an admin records a reviewed reset. Reconcile stranded `started` provider attempts as unknown outcomes without claiming a refund, zero usage, or a billable charge.

AgentRun-linked reconciliation remains age-gated by the hard-limit-plus-grace boundary and requires a finished AgentRun. For request-local work, age alone is never an execution-end signal. PR #194 proves one accepted pattern: the real request owner emits durable completion evidence keyed by the existing operation identity, after which a still-`started` attempt may reconcile to `outcome_unknown` with an atomic status guard and audit. The next extension must wire only another real owner boundary—starting with business advisory—and must not invent a WorkItem, AgentRun, generic timeout, provider failure class, refund, or zero-spend claim. Unknown outcomes retain their authorized call slot.
